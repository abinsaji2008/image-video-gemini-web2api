...
async def publish_raw_blob(
    ...
    *,
    ...
    """Upload raw bytes to the connected public Vercel Blob store."""
    blob_token = os.getenv("BLOB_READ_WRITE_TOKEN", "").strip() or os.getenv("VERCEL_BLOB_READ_WRITE_TOKEN", "").strip()
    oidc_token, store_id = blob_oidc_credentials(oidc_token)

    if oidc_token and store_id:
        normalized_store_id = store_id[len("store_"):] if store_id.startswith("store_") else store_id
        blob_api_url = f"https://vercel.com/api/blob/?{urlencode({'pathname': pathname})}"
        ...
        def upload_oidc() -> dict[str, Any]:
            req = UrlRequest(blob_api_url, data=raw, method="PUT", headers=headers)
            try:
                with urlopen(req, timeout=120) as response:
                    return json.loads(response.read().decode("utf-8"))
            except HTTPError as exc:
                detail = exc.read().decode("utf-8", errors="replace")
                raise RuntimeError(f"Vercel Blob OIDC upload failed (HTTP {exc.code}): {detail}") from exc
            except URLError as exc:
                raise RuntimeError(f"Vercel Blob OIDC upload connection failed: {exc.reason}") from exc
    ...
    if blob_token:
        async with AsyncBlobClient(token=blob_token) as blob_client:
            uploaded = await blob_client.put(
                pathname, raw, access="public", content_type=content_type, add_random_suffix=True
            )
        return str(uploaded.url)

    raise RuntimeError("Vercel Blob is not configured. Connect a public Blob store or configure BLOB_READ_WRITE_TOKEN.")


async def generate_official_image(
    prompt: str,
    model_name: str | None = None,
    *,
    oidc_token: str | None = None,
) -> dict[str, Any]:
    """Generate an image through Google's official Gemini Developer API."""
    import base64

    api_key = get_official_gemini_api_key()
    if not api_key:
        raise RuntimeError("Gemini Web image generation is unavailable and GEMINI_API_KEY is not configured.")
    ...
    model = model_aliases.get(requested) or (model_name.strip() if model_name else "gemini-3.1-flash-image")
    ...
    try:
        ...
        for part in list(response.parts or []):
            ...
            pathname = f"gemini/images/{time.strftime('%Y/%m/%d')}/official-{time.time_ns()}{ext}"
            url = await publish_raw_blob(raw, pathname, mime_type, oidc_token=oidc_token)
            return {
                "url": url,
                "download_url": public_download_url(url),
                "title": "[Official Gemini Generated Image]",
                "alt": "",
                "type": "generated",
                "mime_type": mime_type,
